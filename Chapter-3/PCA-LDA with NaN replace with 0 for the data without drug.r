...
biplot(pc,xlab="PC1 31.83%", ylab="PC2 12.55%",main="PCA biplot for data without drug with NaN replace with 0")
plot3d(X[,1],X[,2],X[,3],type="s",col=co,xlab="PC1 31.83%", ylab="PC2 12.55%",zlab = "PC3 8.79%")

fit = lda(Y ~ X)
fScores = X %*% fit$scaling

plot(fScores[,c(1,2)], type = "p", xlab = "LD1", pch = pch1, col = co, ylab = "LD",cex = 1.5)
