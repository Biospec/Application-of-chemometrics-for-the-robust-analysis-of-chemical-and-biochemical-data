...
n = 19 # number of components
  mean.mydata = apply(mydata,2,mean)

mydata = (mydata-mean.mydata)/ mean.mydata
...
plot(fScores[,c(1,2)], type = "p", xlim = c(xmi, xma), ylim = c(ymi, yma), xlab = "LD 1", pch = pch1, col = co, ylab = "LD 2", main = sprintf("(C) PC-LDA based on Level scaling"),cex=1.5,lty=2,cex.lab = 1.5,cex.main=1.5)
