...
n = 23 # number of components
  mean.mydata = apply(mydata,2,mean)
  std.mydata = apply(mydata,2,sd)
  min.mydata = apply(mydata,2, min)
  max.mydata = apply(mydata, 2, max)

mydata = (mydata-mean.mydata)/ (max.mydata-min.mydata)
...
plot(fScores[,c(1,2)], type = "p", xlim = c(xmi, xma), ylim = c(ymi, yma), xlab = "LD 1", pch = pch1, col = co, ylab = "LD 2", main = sprintf("(B) PC-LDA based on Range scaling"),cex=1.5,lty=2,cex.lab = 1.5,cex.main=1.5)
